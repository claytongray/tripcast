...
_Code review 2026-06-30 (Epic 4 adversarial pass)_

- [ ] [Review][Patch] Shadow Claude call is synchronous on the live send path — when `narration.shadow` is on, `narrateSafely(app(ClaudeNarrator::class), …)` makes a real ~5s Anthropic HTTP call before `deliver()`, blocking every send; contradicts the "never delay the send" docblock. **Decision (2026-06-30): offload the shadow comparison to a separate queued job dispatched after the digest is sent**, so the live send path has true zero added latency and in-prod shadow data is preserved [app/Jobs/SendTripDigest.php:135-136]
- [ ] [Review][Patch] Narration SETUP is not guarded — only `Narrator::narrate()` is wrapped by `narrateSafely`; the prior-snapshot query, `NarrationContext` build (`user->temperature_unit`, `departure_date->toDateString()`), and `app(ClaudeNarrator::class)` resolution run unguarded inside `handle()`. A null user / DB error there fails the job AFTER snapshot persist but before send — the "enhancement-only" feature can break the live digest. Wrap the whole `narrate()` body in the guard [app/Jobs/SendTripDigest.php narrate() + call site]
- [ ] [Review][Patch] "Since yesterday" wording assumes the prior snapshot is exactly 1 day old — the prior is "most recent earlier snapshot," which can be several days old after a failed/paused day or a purge; the delta is real but the temporal label is false. Make the wording reflect actual elapsed time (e.g. "since your last forecast") or constrain the prior to yesterday [app/Services/Narration/DeterministicNarrator.php:471,478; ClaudeNarrator.php; SendTripDigest prior lookup]
- [ ] [Review][Patch] The Anthropic SDK call path is untested — tests cover only the no-delta / no-key early returns; the actual `messages->create(...)`, `TextBlock` parsing, SDK namespaces, and model id (`claude-haiku-4-5`) are never exercised, so a wrong signature/model fails only in prod shadow mode and is swallowed by `catch (Throwable)`. Add a mocked-client test [tests/Feature/Narration/ClaudeNarratorTest.php]
- [ ] [Review][Patch] `narrator:compare` bills the Anthropic API regardless of the `shadow` flag — it only checks for an API key (no shadow gate, `--dry-run`, or confirmation); anyone running it with a key set makes up to `--limit` real billable calls. Add a guard/confirmation [app/Console/Commands/CompareNarrators.php:110-112]
- [x] [Review][Defer] `notable.temp` threshold applied identically to °C and °F — Celsius users get a higher effective bar (10°C ≈ 18°F) so fewer temp lines; needs a unit-aware threshold [app/Services/Narration/NarrationDiffer.php:578,601-604] — deferred, sensitivity tuning
- [x] [Review][Defer] Duplicate date in a snapshot's `days` silently overwrites (last-wins) in `byDate()` — unlikely provider quirk, no dedup/detection [app/Services/Narration/NarrationDiffer.php:626-634] — deferred, low likelihood
- [x] [Review][Defer] `narrator:compare` null-derefs on an orphaned trip user (`$trip->user->temperature_unit`) — console-only; `withTrashed()` applies to trips, not the user relation [app/Console/Commands/CompareNarrators.php:102] — deferred, console-only
- [x] [Review][Dismiss] `{!! $narration !!}` in the text email — plaintext template (HTML escaping would corrupt plain text with entities); narration is grounded (no user free-text), no XSS sink. Raw is correct here [resources/views/emails/digest-text.blade.php]
- [x] [Review][Dismiss] Narration tests under `tests/Feature/Narration` vs spec's `tests/Unit` — internally consistent (file list updated) and co-location enables the shared `snap()`/`fday()` helpers; behavior/coverage unaffected
- [x] [Review][Dismiss] Anthropic SDK pinned `^0.31.0` vs spec's `^0.32.0` — `^0.32` is not published; `^0.31` is the available max. Justified
