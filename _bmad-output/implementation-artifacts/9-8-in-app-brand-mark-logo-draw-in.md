...
- [x] **Task 1 — `BrandMark.vue` component** (AC: 1)
  - [x] New `resources/js/components/BrandMark.vue`. Geometry from `public/brand-assets/tripcast-favicon-32.svg` (`viewBox="0 0 64 64"`, `fill="none" stroke-linecap="round" stroke-linejoin="round"`, stroke-width 3.4): sun circle `cx=32 cy=32 r=11`, five ray strokes, wave `M8 49 q8 -5 16 0 t16 0 t16 0`.
  - [x] Colors: wave stroke = `currentColor` with the component root classed `text-brand` (token `--color-brand`: `#2563a6` light / `#6ba7e0` dark — dark mode is free, do NOT hardcode blue); sun + rays stroke = literal `#E0993D` (no token exists; it reads fine on both washes — same call as the favicon).
  - [x] Size via class from the parent (`size-*`), SVG `class="h-full w-full"` — fixed box, no layout shift. Accessibility: decorative by default (`aria-hidden="true"`), it always sits beside the visible "tripcast" wordmark.
  - [x] Props: `{ animate?: boolean }` (default `false`) — only the landing hero + layout headers pass `animate`; other future uses render static.
- [x] **Task 2 — Draw-in animation** (AC: 2, 3)
  ...
  - [x] Keyframes animate FROM hidden TO the natural (drawn) state, so the element's static/final state is fully drawn — this is what makes reduced-motion and the no-animate path free: `@media (prefers-reduced-motion: reduce) { animation: none }` on all animated parts satisfies AC3 with no JS branch.
  - [x] **Once per hard load — SSR-safe (read carefully, this is the trap):** the app builds an SSR bundle (`npm run build:ssr`, Inertia SSR daemon). Module-scope state on the server persists ACROSS requests, so a naive `let hasPlayed = false` at module scope would flip once on the server and suppress the animation for every later visitor. Guard all flag reads/writes to the client (`typeof window !== 'undefined'` or `!import.meta.env.SSR`); during SSR always render the animate class (a hard load is exactly when it should play). Client-side, read+set the flag synchronously in component setup, and compute the class once (not reactively) so hydration matches the server HTML. Inertia page navigations that remount a layout re-run setup, find the flag set, and render static — that's the AC2 "never replays" path.
- [x] **Task 3 — Swap the surfaces** (AC: 1)
  - [x] `resources/js/layouts/auth/AuthSimpleLayout.vue`: replace the `AppLogoIcon` import + usage (lines 3, 28–31) with `BrandMark` (keep the `size-9` box and the `sr-only` title).
  - [x] `resources/js/layouts/AppLayout.vue`: inside the existing home `<Link>` (lines 17–22), add the mark before the "tripcast" text — mark + wordmark on one baseline (`inline-flex items-center gap-2`, mark ~`size-5`). Wordmark text stays exactly `tripcast` (lowercase — locked voice).
  - [x] `resources/js/pages/Landing.vue`: hero `<h1 class="text-display text-ink">tripcast</h1>` (line 128) becomes the lockup — mark (~`size-10 md:size-12`) beside the word, centered (`flex items-center justify-center gap-3`), `tripcast` text unchanged, still inside the `h1`. Pass `animate` here (and in the two layouts — the module flag arbitrates which one actually plays).
  - [x] Delete `resources/js/components/AppLogo.vue` (unused — nothing imports it; recon-verified) and `resources/js/components/AppLogoIcon.vue` (its only consumer is the AuthSimpleLayout line replaced above). After deletion: `grep -ri "AppLogo" resources/js` must return nothing.
- [x] **Task 4 — Rot-guard test + gates** (AC: 1)
  ...
  - [x] **Gates:** `php artisan test --compact`; `npm run lint:check`; `npm run format:check` (run `npm run format` first); `npm run types:check`; `npm run build` (catches SFC compile errors; SSR bundle: `npm run build:ssr` must also pass because of the Task 2 SSR guard).
  - [x] Browser verification (manual or via available browser tooling): hard load `/` → draw-in plays once; navigate away and back (Inertia) → no replay; hard reload → plays again; OS reduced-motion on → static; dark mode → wave is the light blue token. Ask the builder to run `npm run dev`/`composer run dev` if the change isn't visible.
...
- **Red→green:** `BrandMarkTest` (2 rot-guards: no "Laravel Starter Kit" string under resources/js; AppLogo/AppLogoIcon gone + BrandMark exists) failed against the starter kit, passed after the swaps. Full suite **492 passed** (1976 assertions); pint clean; phpstan 0 errors; vue-tsc clean; eslint clean; `npm run build:ssr` green.
- **BrandMark.vue** built as specced: wave `currentColor` on a `text-brand` root (dark token verified live at `#6ba7e0`), sun/rays literal `#E0993D`, `pathLength="1"` dash-draw for wave (0–500ms) + circle (150–550ms), five rays pop staggered 480–680ms+300ms (ends 980ms), `backwards` fill so hidden-through-delay, static/final state identical to no-animation state, reduced-motion = `animation: none`.
- **Once-per-load flag** lives in a plain `<script>` module block, read/written only client-side (`import.meta.env.SSR` guard) and claimed synchronously in setup so SSR HTML and hydration agree.
- **Browser-verified (chrome-devtools on localhost:8000, dark mode):** hard load `/` → hero lockup draws in (`brand-draw` present); Inertia nav to `/login` → auth-layout mark renders static (no replay); hard reload → plays again; console free of errors/hydration warnings. Note: session began at 2bbd2f8 (baseline preserved); dev ran on the post-merge tree (ee863f7) — AppLayout's Epic 7 admin-link restructure was re-reconned before the swap.
- Surfaces: AuthSimpleLayout (mark replaces AppLogoIcon, size-9 box + sr-only title kept), AppLayout header (mark + lowercase wordmark in the home link), Landing hero h1 → mark + word lockup. `AppLogo.vue` (dead code) and `AppLogoIcon.vue` deleted; zero references remain.

### File List (recorded)

**New:**
- `resources/js/components/BrandMark.vue`
- `tests/Feature/Landing/BrandMarkTest.php`
...
- 2026-07-02 — Story 9.8: BrandMark component (sun+wave, dark-mode aware) replaces all starter-kit branding across auth layout, app header, and landing hero; pure-CSS once-per-hard-load draw-in (<1s, SSR-safe module flag, reduced-motion static); rot-guard test added. Full suite 492 passed; all frontend + PHP gates green; behavior browser-verified.
