...
1. **Given** a signed-in user on the dashboard, **when** the page renders, **then** a feedback card appears **below the "Send a sample" card, as the last section on the page** (always visible, like the sample card), with heading "Thoughts? Ideas? Please send them.", one warm supporting line about feedback shaping tripcast early on, a ~3-row textarea, and a "Send feedback" button. *(Amended 2026-07-03 during browser check: Clayton moved it from between past-trips and the sample card to below the sample card.)*
