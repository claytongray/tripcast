...
- [ ] **Task 5 — Resolve the MailerSend List-Unsubscribe gate `#MS42235`** (AC: 3): decide **plan upgrade** (custom `List-Unsubscribe` headers allowed — keeps the current `DigestMail::headers()` implementation untouched, the clean option) **vs. MailerSend's built-in unsubscribe** (would substitute their header/landing page — verify it still emits RFC 8058 one-click before choosing it). Open retro action **A2** tracks this. After resolving: send a real `DigestMail` and confirm **202 accepted, no 422**, and the received message carries `List-Unsubscribe` + `List-Unsubscribe-Post: List-Unsubscribe=One-Click`.
...
1. **MailerSend `#MS42235`** — `DigestMail` 422s on send until the plan gate is resolved (Task 5; retro action A2). The header must NOT be dropped to "fix" the 422 — it's a Gmail/Yahoo bulk-sender requirement (NFR-2/UX-DR17).
